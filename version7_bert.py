...
X_train = list(map(lambda x: tokenizer.encode(x, max_length=MAX_LEN, pad_to_max_length=True), X_train_raw))
X_test = list(map(lambda x: tokenizer.encode(x, max_length=MAX_LEN, pad_to_max_length=True), X_test_raw))
with open('data/bert_input.pkl', 'rb') as f:
    X_train, X_test = pickle.load(f)
"""
with open('data/bert_input.pkl', 'rb') as f:
    X_train, X_test = pickle.load(f)
X_train, X_valid, y_train, y_valid = train_test_split(X_train, y_train, test_size=0.1, random_state=0, shuffle=True)
...
submission[classes] = predictions
submission.to_csv("submission/v7_bert_2.csv.gz", compression="gzip", index=False)